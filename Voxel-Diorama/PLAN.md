...
D12 ORG v4 (2026-07-27, 사용자 지시 — 외부 3안 채택): 반복되는 Opus 세션 한도 소진 대응으로 Meta-Loop 재편. **Opus 5=Board Advisor(온디맨드 크리틱, 핫패스 밖 — 자문은 CLI 크로스-리뷰가 수행, ThreeJSDev paused)** — **단 '핫패스 밖'은 기본값이지 금지가 아니며, 중요·핵심 부분(핵심 아키텍처·데이터 계약·보안/승인 경로·복구 로직, 또는 하위 모델이 2회+ 반려된 난제)을 코딩할 때는 Opus 5를 Worker(구현자)로 직접 투입한다**, **Codex(sol/terra)=Chief Operator/Senior(핫패스 주력)**, **Workers=Sonnet 5/Haiku 4.5**(구 Gemini 대체, 유료 API 미사용). 모델 라우터=복잡도+비용. **오케스트레이션은 CLI+CEO 유지**(Lean B가 에이전트 간 배정을 막으므로 에이전트 오케스트레이터 불가). 상세·라우터·Graph-of-Loops 개념 매핑은 PAPERCLIP-RECON.md "ORG v4" 참조.
D12 ORG v5 (2026-07-28, 사용자 지시 — sol>terra>luna 서열 정정 + codex-plugin-cc 도입): **최상위 모델(sol)을 구현이 아니라 적대 검수 게이트에 둔다.** 근거 — sol이 구현한 VOX-25는 1·2라운드 연속 결함, terra의 검수 1회가 실제 결함 4건 검출. 그리고 D9 때문에 오판이 영구적이라 되돌릴 수 없는 쪽은 검수다. 배치: **적대 검수 게이트=`gpt-5.6-sol`(CLI 플러그인, 에이전트 아님)** · B 복잡 구현=`gpt-5.6-terra`(CodexDev) · A 단순반복=`gpt-5.6-luna`(paused) · C QA기본/구현겸업=`claude-sonnet-5`(Developer) · 체크포인트 QA=terra(CodexQA) · 핵심 에스컬레이션=`claude-opus-4-8`(ThreeJSDev, paused) · Advisor=`claude-opus-5`(CLI 세션) · Technical Director **paused**(sol 자리를 게이트가 가져갔고 분해는 CLI+CEO가 수행, 실사용 0회). 신규 규칙 **R-1 한 업무에서 구현자와 QA는 반드시 다른 provider** / **R-2 모델 변경 후 실제 런 로그에서 모델명 육안 확인(설정 리드백은 검증이 아님)**. 게이트가 Lean B를 위반하지 않는 이유는 `adversarial-review`가 **읽기 전용**이라 승인·커밋·자가배정 수단 자체가 없기 때문. 상세는 PAPERCLIP-RECON.md "ORG v5" 참조.
-->
...
13. **지오-리얼리즘 — OSM 실제 지도로 섬 지형 생성** — 섬을 빈 평면이 아니라 "압축된 실제 도시"(간선 도로망·강·랜드마크)로 만든다. 설계·분석 전문: `docs/geo-realism.md` (little-city/little-taipei `plans/realism-plan.md` 실사 분석). 4단계 권고: **G1 도구 3종**(`fetch-geo-truth`→`geo-truth.json` 커밋 / `check-city` 존별 드리프트 strict 실패 / `map-preview` 게임-vs-OSM 대조 SVG) — 앱 코드 미접촉·의존성 0이라 **선행 착수 가능**, **G2** 대조 SVG를 배치 업무 검수 첨부물 표준으로(§3.2 완료 조건 강화), **G3** `city.js` 스키마 + 도로·강 렌더(Phase 5), **G4** D8 슬롯 생성기 교체(D8 개정 후, Phase 5). 원칙: **데이터는 진짜 km, 왜곡은 런타임 `spread` 한 곳** / 불변식은 방위·위상이지 거리가 아님 / 충실도 존(A 0.15km · B 0.4km · C 방위만). 라이선스 ODbL — `THIRD_PARTY.md` 등재 필요.
    <!-- 2026-07-27 사용자 지시로 레퍼런스 3종(little-taipei·mykonos·tiny-world-builder) 연구. 지금 G3부터 시작하면 Phase 3(루프 완성)이 밀려 §1 도그푸딩 완주가 밀림 → 도구(G1)만 조기, 렌더는 테마 작업에 합류시키는 것이 권고. Overpass 실측: 504 빈발, 타임아웃 ≥100초 + 엔드포인트 다중화 필수. -->
...
4. **D8 개정 여부 (인간 결정)** — §9-13 지오-리얼리즘 G4를 채택하면 D8 문구 "부지 = 섬 중앙→바깥 **결정적 나선**"을 "**결정적 부지 목록**(도시 블록에서 유도, 앵커 거리→각도 순 정렬)"로 개정해야 한다. 결정론과 '중앙→바깥' 의미는 유지되고 교체 대상은 슬롯 생성기 함수 하나(VOX-25 산출물 보존). 나선은 도로망이 없는 `base` 테마 폴백으로 잔존. 근거·대안은 `docs/geo-realism.md` §3-2·§5.
