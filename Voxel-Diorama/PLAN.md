...
| D4 | 인간 전용 권한 | 승인·거절·여행 종료는 사용자만. AI 금지 목록은 §4.1. **강제(2026-07-22 실사 확정)**: 에이전트는 기본적으로 승인 없이 자율 진행하므로, 사원에 `trustPreset: low_trust_review`(산출물 격리→인간 promote) + `canAssignTasks: false`(자율 하위작업 폭주 차단)를 설정해 **검수 전 정지(B)**를 강제 — PAPERCLIP-RECON.md 게이팅 절 참조. 게임 층 §3.4-4('검수 우회 done→조정 필요')는 이중 방어로 유지 |
