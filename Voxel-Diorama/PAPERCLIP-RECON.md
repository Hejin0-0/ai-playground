...
- **검수 API 실체** (2026-07-28 실측, v2026.722.0): `POST /api/tasks/{id}/review` 는 **없다(404)**. `POST /api/issues/{id}/approvals` 는 존재하지만 `approvalId`를 요구한다 — 즉 **이미 올라온 승인 요청에 결정을 내리는 것이지 승인 레코드를 새로 만드는 게 아니다**. 승인 요청이 없는 이슈는 `GET .../approvals` 가 `[]`. 따라서 CLI에서의 반려는 (1) CEO 코멘트로 사유·재작업 완료조건 기록 → (2) `todo` 되돌림 → (3) 담당자 사이클로 재발주, 이 3단계로 집행한다. ⚠️ `src/hud/review.ts`의 `createReviewSubmitter`가 없는 라우트를 호출하고 있다(백로그).
